...
class MeshtasticApp:
    ...
    def _build_aide_tab(self, parent):
        ...
        ttk.Label(inner, text="📖  User Guide d'utilisation — Meshtastic Config Manager v1.2",
        ...
        para("By default, JSON files are saved in the script/EXE folder.")
        step(1, "Click Choose to select a different folder.")
        step(2, "Click Open to open it in File Explorer.")

        section("Device connection")
        para("The COM port field lets you target a specific port or leave it blank for auto scan.")
        step(1, "Connect the device via USB.")
        step(2, "Click Detect ports — the first detected port is pre-selected.")
        step(3, "Leave the field empty to scan all ports automatically.")
        para("If no port is detected: check the cable and the CP210x / CH340 drivers.")

        section("Export configuration")
        para("Reads and saves the full configuration of the connected device to a JSON file.")
        step(1, "Make sure the device is connected.")
        step(2, "Click EXPORT FULL CONFIG.")
        step(3, "Choose the name and location of the backup file.")
        step(4, "Wait for the Export successful confirmation message.")
        para("Contents: owner, LoRa, Bluetooth, network, position, power, modules, channels, nodes.")

        section("Sequential multi-node export")
        para("Exports multiple nodes in sequence without leaving the application.")
        step(1, "Click MULTI-NODE EXPORT.")
        step(2, "Connect each node in turn and confirm at each step.")
        step(3, "Click No to finish — a summary is shown.")

        section("Backup file management")
        para("The list shows: type, model, channel, date. Sorted by date descending.")
        step(1, "Refresh — reloads the file list.")
        step(2, "View content or double-click — opens the JSON read-only.")
        step(3, "Copy file — duplicates the selected file.")
        step(4, "Delete — permanently deletes (confirmation required).")
        para("BACKUP = full backup (green)   FLEET = fleet profile (blue)")

        section("Generate a Fleet Profile")
        para("A fleet profile is a stripped-down config, deployable on any node.")
        step(1, "Select a full export file from the list.")
        step(2, "Click GENERATE FLEET PROFILE.")
        step(3, "Choose the name for the fleet profile file.")
        para("REMOVES: pub/priv key, identifier, owner, nodes, WiFi credentials, counters.")
        para("KEEPS: admin_key, LoRa, channel PSKs, modules, BT, display, position, power.")

        section("Key fields editor")
        para("Edit key fields of a JSON file without an external editor.")
        step(1, "Select a file from the list.")
        step(2, "Click Edit key fields.")
        step(3, "Edit: long/short name, LoRa region, modem preset, primary channel name.")
        step(4, "Click Save — the JSON file is updated directly.")


        section("Restore configuration")
        para("Writes a JSON file back to a connected device.")
        step(1, "Select the file from the list OR Browse for file.")
        step(2, "Click RESTORE.")
        step(3, "If integrity warnings appear, read them before confirming.")
        step(4, "Wait for success, then RESTART the device to apply.")
        para("WARNING: restore overwrites the current device config. Export first if needed!")

        section("Multi-node import (sequential deployment)")
        para("Deploys the file selected in the list to multiple devices in sequence.")
        step(1, "Select the target JSON file from the file list.")
        step(2, "Click MULTI-NODE IMPORT.")
        step(3, "For each device: connect it, select its port (COM1 excluded), click Restore.")
        step(4, "Click Finish when all nodes have been processed.")
        para("Note: COM1 is automatically excluded from detection (Windows system port).")
        para("Note: the selected file is identical for all nodes. With a fleet profile,")
        para("  private/public keys will not be restored — each node keeps its own.")

        section("Integrity check")
        para("Before any restore, the file is automatically validated:")
        para("* Presence of local_config and local_config.lora")
        para("* Presence of the channels section")
        para("* LoRa config consistency (region, modem_preset)")
        para("If a problem is found, a warning is shown — you can cancel or proceed.")

        section("Tips and best practices")
        para("* Name your files explicitly: meshtastic_backup_meeting_2025.json")
        para("* Always export BEFORE modifying or applying a fleet profile.")
        para("* Fleet profiles can be shared among team members.")
        para("* If export fails with Timeout: unplug/replug the cable and try again.")
        para("* On Windows: check Device Manager > Ports (COM & LPT).")
        para("* The JSON file is human-readable and can be edited manually in any text editor.")

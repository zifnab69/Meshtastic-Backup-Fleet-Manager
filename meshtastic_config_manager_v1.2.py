...
class MeshtasticApp:
    ...
    def _build_ui(self):
        ...
        tk.Button(ef, text="📤 EXPORTER LA CONFIGURATION COMPLÈTE → JSON",
                  command=self.export_config,
                  background="#cce0ff", activebackground="#99c2ff",
                  relief="raised", font=("Arial", 9, "bold")).pack(fill="x", ipady=6)
        ttk.Button(ef, text="📤 Export multi-nœuds (séquentiel)",
        ...
        ttk.Button(br, text="👁 Voir contenu",       command=self.view_file).pack(side="left", padx=3)
        ttk.Button(br, text="✏ Éditer champs clés", command=self.edit_config_fields).pack(side="left", padx=3)
        ttk.Button(br, text="📋 Copier fichier",     command=self.copy_file).pack(side="left", padx=3)
        ttk.Button(br, text="🗑 Supprimer",          command=self.delete_file).pack(side="left", padx=3)
        ttk.Button(br, text="📂 Choisir un autre fichier…", command=self.import_browse).pack(side="left", padx=(12, 3))

        # Profil flotte
        ff = ttk.LabelFrame(root, text="  🚀 Profil Flotte — Générer une configuration déployable  ", padding=8)
        ff.pack(padx=20, pady=4, fill="x")
        ttk.Button(ff, text="🚀 GÉNÉRER LE PROFIL FLOTTE depuis le fichier sélectionné",
        ...
        ttk.Button(imf, text="📥 Restauration multi-nœuds (déploie séquentiellement un fichier sur plusieurs appareils)",
    ...
    def _build_aide_tab(self, parent):
        ...
        para("* Nommez vos fichiers explicitement : meshtastic_backup_festival_2025.json")

#!/usr/bin/env python3
"""
Nodes Backup & Fleet Manager v1.77
...
def read_file_meta(path: Path) -> dict:
    ...
    try:
        ...
        known_nodes_count = len(known) if isinstance(known, list) else 0
    ...
    "fr": {
        ...
        "help_sections": [
            ...
            ("Éditeur de champs clés", [
                ...
                ("s",(7,"Générateur de clé : choisissez la taille (Défaut / 128 bits / 256 bits), cliquez Générer. Utilisez le bouton 📋 du champ résultat pour copier la clé, puis collez-la dans le champ PSK souhaité.")),
    ...
    "en": {
        ...
        "help_sections": [
            ...
            ("Key fields editor", [
                ...
                ("s",(7,"Key generator: choose size (Default / 128 bits / 256 bits), click Generate. Use the 📋 button on the result field to copy the key, then paste it into the desired PSK field.")),
...
class NBFMApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Nodes Backup & Fleet Manager v1.77")
    ...
    def _build_ui(self):
        ...
        ttk.Label(hdr, text="Nodes Backup & Fleet Manager v1.77",
        ...
        def _make_sort(col):
            def _sort():
                # Masquer les en-têtes de groupe et décocher la case
                self._group_visible_var.set(False)
                self._apply_group_visibility()
                # Ne trier que les vraies lignes (pas les group_header)
                data = [
    ...
    def _build_aide_tab(self, parent, T=None):
        ...
        ttk.Separator(inner, orient="horizontal").pack(fill="x", pady=(10, 4))
        ttk.Label(inner, text="Nodes Backup & Fleet Manager v1.77",
...
<div class="footer">Nodes Backup &amp; Fleet Manager — NBFM Report</div>
    ...
    def edit_config_fields(self):
        ...
        KEY_SIZES = [("Default (AQ==)", 0), ("128 bits", 16), ("256 bits", 32)]  # tailles acceptées par Meshtastic
